...
var (
	...
	frontendPath  = flag.String("frontend", "./frontend/dist", "前端文件路径")
...
func (l *Launcher) startBackend() error {
	...
	l.backendCmd = exec.CommandContext(l.ctx, backendPath, "--port", *backendPort)
...
func (l *Launcher) waitForBackend() error {
	backendURL := fmt.Sprintf("http://localhost:%s/api/v1/health", *backendPort)
...
func (l *Launcher) startIntegratedServer() {
	...
	// 配置API代理
	apiURL, _ := url.Parse(fmt.Sprintf("http://localhost:%s", *backendPort))
	...
	mux.HandleFunc("/docs", func(w http.ResponseWriter, r *http.Request) {
		http.Redirect(w, r, fmt.Sprintf("http://localhost:%s/docs", *backendPort), http.StatusTemporaryRedirect)
	})
